...
const REFERENCE_HASHES={"homework":{"accepted":["255b05eb53740d533631973391b0a3e9ba6dee14dfc71bc1ec3a4b91339a7652"],"rejected":["61e09ae68a6bdfbbff6287012cd2aa50aad16a6e41504d91b70b5643b7788f29"]},"hangout":{"accepted":["2f8761ce5250ee7cf15e823adf9e1f39b82076acf3fe6d7d52f0bb765307419e"],"rejected":["ab31a9e7b88433058faddc2f6a1f036f6cbf4e2c5c401f71cbeef604b82f9351"]},"nap":{"accepted":["686849ded487115881f1fa03801ea95d80db6b70d7f7c60ddeb74857def61a82"],"rejected":["d9915b6e3d7f412ba21a5e66569c99e6380e33714ee60196d856d5f570ffe2f5"]},"workout":{"accepted":["f1f11c04563c3218d644c8e8aae4312a960fe7310cf2e50db291037b33bc03a2"],"rejected":["800a0f2c495a4bf5bbb36d66fba5cf55ab20e9113abb3f31549f8e6c122fe564"]}};
...
async function verifyAgainstReferenceImages(file,category){const hash=await sha256File(file);const r=REFERENCE_HASHES[category]||{accepted:[],rejected:[]};if(r.rejected.includes(hash))return {verified:false,reason:"This image is on the rejected reference list."};if(r.accepted.includes(hash))return {verified:true,reason:"This image matches an approved reference image."};return {verified:false,reason:"This image is not one of the approved reference images."}}
...
async function verifyTask(){
 ...
 const referenceReady=(REFERENCE_HASHES[category]?.accepted?.length||0)+(REFERENCE_HASHES[category]?.rejected?.length||0)>0;
 if(referenceReady){
   const result=await verifyAgainstReferenceImages(file,category);
   if(result.verified){state.tasks[currentTask].done=true;state.points++;closeModal();render();save();toast("Verified ✓ Your parents sent you this point",4500);checkReward();}
...
let unlockedLevels=Math.max(1,Number(localStorage.getItem("threeDoUnlockedLevels")||1));
let completedLevels=(()=>{try{return JSON.parse(localStorage.getItem("threeDoCompletedLevels")||"[]")}catch{return []}})();
if(!Array.isArray(completedLevels))completedLevels=[];
let selectedLevel=Number(localStorage.getItem("threeDoSelectedLevel")||state.level||1);
function saveGameProgress(){
 localStorage.setItem("threeDoUnlockedLevels",String(unlockedLevels));
 localStorage.setItem("threeDoCompletedLevels",JSON.stringify(completedLevels));
 localStorage.setItem("threeDoSelectedLevel",String(selectedLevel));
...
function markLevelCompleted(level){
 ...
 unlockedLevels=Math.max(unlockedLevels,Math.min(30,level+1));
 selectedLevel=Math.min(30,level+1);
 saveGameProgress();
 renderLevelMenu();
}
...
load();
unlockedLevels=Math.max(unlockedLevels,Math.min(30,Number(state.level||1)));
