...
function render(){
 ...
 for(let i=0;i<state.slots;i++){
  ...
  row.innerHTML=`<button class="check" ${t.done?"disabled":""} onclick="openProof(${i})">${t.done?"✓":""}</button><input class="task-name" ${t.done?"disabled":""} value="${esc(t.text)}" placeholder="Write Task ${i+1}" onchange="updateTask(${i},this.value)"><small>${t.done?"Completed":"Task "+(i+1)}</small>`;
...
async function verifyTask(){
 ...
 let category=null;
 if(/home\s*work|homework|study|school|assignment|worksheet|واجب|مذاكر|مذاكرة|دراسة|دروس/.test(lower))category="homework";
 else if(/hang\s*out|hangout|outside|street|walk|go out|خروج|شارع|فسحة|تمشية|بره|خارج/.test(lower))category="hangout";
 else if(/take\s*a\s*nap|nap|sleep|rest|bed|نوم|قيلولة|سرير|راحة/.test(lower))category="nap";
 else if(/workout|work\s*out|gym|exercise|training|fitness|جيم|تمرين|تمرينات|رياضة/.test(lower))category="workout";
